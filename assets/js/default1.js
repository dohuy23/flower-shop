...
<div class="product">
        ...
        <img class="product-image" src="${detail[0].image}" alt="ảnh">
    ...
    <div class="product-content">
        ...
        <div class="product-add">
            ...
            <button class="cart1">Thêm vào giỏ hàng</button>
...
// gọi biến title chung
let title = detail[0].title;
// console.log(title);
// truy câp phần tử
let rowJsHt = document.querySelector(".row-js-ht")

// lọc title
const getDataDetail = (list, value,productId) =>{
  return list.filter((item)=>{
    return item.title === value && item.id != productId;
  })
};
const lisData = getDataDetail(data, title, id)
// console.log(lisData);
// hien thi du lieu
const renderDataDetail = (lisData) =>{
  let UML = ``;
...
rowJsHt.innerHTML = renderDataDetail(lisData)
...
// Style lại web
  const description = document.querySelector(".tittle-coment");
const comentTt = document.querySelector(".coment")
const iClass = document.querySelector(".fa-solid")
  // console.log(iClass);


description.addEventListener('click',() =>{
comentTt.classList.toggle("show");

})
 
// làm nút thêm vào giỏ hàng
// truy cập phần tử
const buyToCart = document.querySelector(".cart1")
// console.log(buyToCart);
buyToCart.addEventListener("click",()=>{
  cartModalOverlay.style.transform = "translateX(0)";
 
  // console.log(quantityBt);
  buytoButtonClick (buyToCart)
});
const buytoButtonClick = (item) =>{

const parentButtonEl = item.closest('.product');
const imgBtt = parentButtonEl.querySelector(".product-image").src;
const nameBtt = parentButtonEl.querySelector(".product-name").innerHTML;
const priceBtt = parentButtonEl.querySelector('.product-price').innerHTML;
const quantityBt = parentButtonEl.querySelector('.product-quantity').value
// console.log(priceBtt);

const isDuplicate = cartItem.some(item => item.name === nameBtt);
...
if (!isDuplicate) {
  ...
  // Lưu giỏ hàng vào local storage và hiển thị lại giỏ hàng
  saveCartToLocalStorage(cartItem);
  addItemToCart(cartItem);
} else {
